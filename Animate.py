...
# The 'col' parameters make assumptions about the size of the rewards you give your agents.
# You may need to change their values to generate an animation with
# clearly identifiable start and end states.
# The final frame of the animation showcases the supplied optimal policy as it
# would be executed by the agent on the given environment.
# The function also assumes that the last record represents the iteration/epoch where,
# in the case of Value Iteration, convergence was DETECTED and
# in the case of Q-Learning, the last record is the last epoch on the learning algorithm

# Don't forget to turn off interpolation when viewing these animations or else each cell
# will not be easily identifiable.
...
def generateAnimat(records: [[float]], start_state: (float, float), end_state: (float, float),
                   mines: [(float, float)] = [], opt_pol: [(float, float)] = [], start_val: int = 0, end_val: int = 255,
                   mine_val: int = 254, opt_val: int = 50,
                   fps: int = 1, vmin=0, vmax=255, generate_gif: bool = False, filename: str = 'animat',
                   just_vals: bool = False):
    fig, ax = plt.subplots()
